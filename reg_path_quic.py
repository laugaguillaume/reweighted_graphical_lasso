...
from benchmark_utils import AdaptiveQuicSolver
...
def synthetic_data(p=50, n=1000, alpha=0.9):
    ...
    return alpha_max, X_train, S_train, S_test, Theta_true
...
smallest_alpha_scaler = 5e-3
grid_size = 20
number_of_runs = 8

# Generate synthetic data
alpha_max, X_train, S_train, S_test, Theta_true = synthetic_data(p=p, n=n, alpha=sparsity)
...
models = [
    ...
    GraphicalLasso(algo="primal",
                   ...
                   warm_start=True,
                   tol=models_tol),
    AdaptiveQuicSolver(strategy="log",
                n_reweights=n_reweights,
                tol=models_tol,
                max_iter=max_iter_reweights),
    AdaptiveQuicSolver(strategy="mcp",
                n_reweights=n_reweights,
                tol=models_tol,
                max_iter=max_iter_reweights),
    # AdaptiveQuicSolver(strategy="scad",
    #             n_reweights=n_reweights,
    #             tol=models_tol,
    #             max_iter=max_iter_reweights),
    AdaptiveQuicSolver(strategy="sqrt",
                n_reweights=n_reweights,
                tol=models_tol,
                max_iter=max_iter_reweights),
...
for i, (penalty, model) in enumerate(zip(penalties, models)):
    for alpha_idx, alpha in enumerate(alphas):
        ...
        model.alpha = alpha
        fit_times = []
        if isinstance(model, AdaptiveQuicSolver):
            for k in range(number_of_runs):
                t0 = time.perf_counter()
                model.fit(S_train,X_train)
...
plt.savefig(
    f"./bench_results/quic/p{p}_n{n}_alpha{sparsity}_{data_type}_max_iter_reweights_{max_iter_reweights}_gridsize_{grid_size}.pdf")
plt.show(block=False)
pd.DataFrame(csv_rows).to_csv(f"./bench_results/quic/p{p}_n{n}_alpha{sparsity}_{data_type}_max_iter_reweights_{max_iter_reweights}_gridsize_{grid_size}_glasso_summary.csv", index=False)
