...
BASEDIR = Path(__file__).resolve().parent / "bench_results" / "quic"
EXCLUDE = {"L1", "MCP", "SCAD", "L0_5", "Log", "R-SCAD"}  # penalties to drop

pattern = "p75_*_alpha0.9_*_max_iter_reweights_*_gridsize_20_glasso_summary.csv"
files = sorted(BASEDIR.glob(pattern))
if not files:
...
for fp in files:
    ...
    df = df.rename(columns={"f1": "max_f1", "best_f1": "max_f1"})
...
for pen, g in full.groupby("penalty"):
    ...
    plt.plot(g["reweights"], g["max_f1"], marker="o", linestyle="-", label=penalties[k])
    k = k +1
...
plt.tight_layout()
plt.savefig(
    f"bench_results/QUIC_reweighting_p_75_n_1000_alpha_0.9_max_F1.pdf")
plt.show()
...
for pen, g in full.groupby("penalty"):
    g = g.sort_values("reweights")
    plt.plot(20*g["reweights"], g["max_f1"], marker="o", linestyle="-", label=penalties[k])
    k = k +1
...
plt.tight_layout()
plt.savefig(
    f"bench_results/QUIC_iterations_p_75_n_1000_alpha_0.9_max_F1.pdf")
plt.show()